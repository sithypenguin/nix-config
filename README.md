...
Multi-host NixOS configuration using flakes with Home Manager integration and declarative dotfile management.
...
- **sithy-one** - Main laptop configuration with Hyprland/Plasma6 support

## Features

- ✅ **Declarative system configuration** with NixOS flakes
- ✅ **Home Manager integration** for user-level packages and dotfiles
- ✅ **Conditional module loading** based on system profiles (laptop/desktop)
- ✅ **Dotfile management** - All Hyprland configs version-controlled and symlinked
- ✅ **Multiple desktop environments** - Easy switching between Plasma6 and Hyprland
- ✅ **Gaming support** - Steam with proper configuration
...
home-manager remove-generations 10 11 12 13 14
...
├── modules/
│   ├── default.nix              # Central module aggregator
│   ├── systemConfig/            # System-level configurations
│   │   ├── host-options.nix     # Custom option definitions
│   │   ├── fonts.nix            # Font configuration
│   │   ├── audio.nix            # Audio/PipeWire config (conditional)
│   │   ├── bluetooth.nix        # Bluetooth config (conditional)
│   │   ├── display.nix          # Display/DE config (conditional)
│   │   ├── networking.nix       # NetworkManager config (conditional)
│   │   ├── sysConfig.nix        # Basic system settings
│   │   └── hyprland.nix         # Hyprland system-level config
│   ├── hyprland/                # Hyprland-specific modules
│   │   ├── cachix.nix           # Binary cache for faster builds
│   │   ├── hyprland.nix         # User packages for Hyprland
│   │   └── hyprland-config.nix  # Dotfile symlink management
│   ├── gaming/
│   │   └── steam.nix            # Steam configuration (conditional)
│   ├── packages/                # User package lists
│   │   ├── sys-util-packages.nix  # System utilities
│   │   ├── gui-packages.nix       # GUI applications
│   │   └── tui-packages.nix       # Terminal applications
│   └── profiles/
│       └── laptop.nix           # Laptop profile - sets mySystem options
└── users/
    ├── users.nix                # System user account definitions
    └── sithy/
        └── home.nix             # Home Manager configuration for user
```

## What Makes This Reproducible

1. **Flake lock** (`flake.lock`) pins exact versions of nixpkgs and home-manager
2. **Declarative config** - Everything is defined in `.nix` files, no imperative commands
...
4. **Add to flake.nix:**
   ...
   nixosConfigurations = {
       # ... existing hosts ...
       new-hostname = mkHost { hostname = "new-hostname"; };
   };
   ```
...
4. **Adjust profile:** Edit `modules/profiles/laptop.nix` or create a new profile
...
8. **Update user info:** Change username in `users/users.nix` and `users/sithy/`
9. **Rebuild:** `sudo nixos-rebuild switch --flake .#your-hostname`
